# ============================================================
# Pancreas Gene Correlation Analysis - Fig_2h
...
genes_fig2h <- c("Mafa", "Ins1", "Ins2", "Insr", "Tbc1d4", "Glut1", "Zfand6", "Srr")

# ============================================================
# 3) Import and prepare data
# ============================================================
cat("\n=== LOADING PANCREAS GENE EXPRESSION DATA ===\n")
dat_raw <- read.csv("Fig_2ef_Pancreas_FoldChange_Data.csv", stringsAsFactors = FALSE)

dat <- dat_raw %>%
  filter(Gene %in% genes_fig2h) %>%
...
cat("Genes:", paste(genes_fig2h, collapse = ", "), "\n")
...
plot_data$Gene1 <- factor(plot_data$Gene1, levels = genes_fig2h)
plot_data$Gene2 <- factor(plot_data$Gene2, levels = rev(genes_fig2h))
...
p_heatmap <- ggplot(plot_data, aes(x = Gene1, y = Gene2, fill = Correlation)) +
  ...
  geom_text(aes(label = Label), size = 2.0, color = "black", lineheight = 0.75) +
  col_palette +
...
ggsave(
  filename = "Fig_2h_Correlation_Matrix_Pancreas.png",
  plot     = p_heatmap,
  scale    = 1, width = 7.5, height = 5.7,
  units    = "cm", dpi = 600, limitsize = FALSE
)

ggsave(
  filename = "Fig_2h_Correlation_Matrix_Pancreas.svg",
  plot     = p_heatmap,
  scale    = 1, width = 7.5, height = 5.7,
...
write.csv(cor_export, "Fig_2h_Correlation_Matrix_Pancreas.csv",    row.names = FALSE)
write.csv(p_export,   "Fig_2h_PValue_Matrix_Pancreas.csv",         row.names = FALSE)
write.csv(n_export,   "Fig_2h_N_Matrix_Pancreas.csv",              row.names = FALSE)
...
write.csv(pairwise_cor, "Fig_2h_Pairwise_Correlations_Pancreas.csv", row.names = FALSE)
...
cat(rep("=", 70), "\n", sep = "")
cat("Figures:\n")
cat("  - Fig_2h_Correlation_Matrix_Pancreas.png (600 dpi, 7.5×5.7 cm)\n")
cat("  - Fig_2h_Correlation_Matrix_Pancreas.svg (vector format)\n")
cat("\nData tables:\n")
cat("  - Fig_2h_Correlation_Matrix_Pancreas.csv    (correlation coefficients)\n")
cat("  - Fig_2h_PValue_Matrix_Pancreas.csv         (p-values)\n")
cat("  - Fig_2h_N_Matrix_Pancreas.csv              (n per pair)\n")
cat("  - Fig_2h_Pairwise_Correlations_Pancreas.csv (pairwise results with n)\n")
cat("\nInterpretation:\n")
