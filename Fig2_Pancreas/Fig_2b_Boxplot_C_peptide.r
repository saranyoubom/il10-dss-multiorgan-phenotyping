...
# "Replicate" column represents individual animal IDs
# Paired t-test used consistently throughout (plot and CSV)
...
# ============================================================
# 4) Import C-peptide data
# ============================================================
cat("\n=== LOADING C-PEPTIDE DATA ===\n")
...
dat <- dat %>%
  ...
  filter(!is.na(C_peptide))

# Convert variables to factors
# IMPORTANT: Replicate is the individual animal ID for pairing
...
# Tests are run for transparency; paired t-test is used
# consistently throughout regardless of normality result
# ============================================================
cat("\n=== NORMALITY TESTING (Shapiro-Wilk) — Informational Only ===\n")
cat("(Paired t-test used throughout regardless of normality result)\n\n")
...
for (grp in z_order) {
  for (comp in my_comparison_weeks) {
    ...
    data_wide <- dat %>%
      ...
      select(Replicate, Week_num, C_peptide) %>%
      pivot_wider(names_from  = Week_num,
                  values_from = C_peptide,
                  names_prefix = "Week_")
...
write.csv(normality_results,
          file      = "Fig_2b_Normality_Tests.csv",
          row.names = FALSE)
cat("✓ Normality results exported (informational only)\n")

# ============================================================
# 5) STATISTICAL TESTS — PAIRED T-TEST THROUGHOUT
#    Raw p-values only; no multiple-comparison correction
# ============================================================
...
for (grp in z_order) {
  for (comp in my_comparison_weeks) {
    ...
    test_result <- compare_means(
      formula     = C_peptide ~ Week_num,
      data        = dat_subset,
      method      = "t.test",     # Paired t-test — consistent throughout
      paired      = TRUE,
...
cat("\n=== Statistical Test Results (raw p-values, paired t-test) ===\n")
print(all_test_results)

write.csv(all_test_results,
          file      = "Fig_2b_Statistical_Tests.csv",
          row.names = FALSE)
cat("✓ Statistical test results exported to Fig_2b_Statistical_Tests.csv\n")
...
combined_plot <- ggboxplot(
  ...
  lims(y = c(-0.4, 3.4)) +
  labs(y = "Fasting C-peptide (ng/mL)", x = "Week") +
...
ggsave(
  filename  = "Fig_2b_Boxplot_C_peptide.png",
  plot      = combined_plot,
...
ggsave(
  filename  = "Fig_2b_Boxplot_C_peptide.svg",
  plot      = combined_plot,
  scale = 1, width = 8.5, height = 5.7,
...
summary_stats <- dat %>%
  ...
  summarise(
    n      = n(),
    mean   = round(mean(C_peptide,   na.rm = TRUE), 3),
    sd     = round(sd(C_peptide,     na.rm = TRUE), 3),
    sem    = round(sd(C_peptide,     na.rm = TRUE) / sqrt(n()), 3),
    median = round(median(C_peptide, na.rm = TRUE), 3),
    min    = round(min(C_peptide,    na.rm = TRUE), 3),
    max    = round(max(C_peptide,    na.rm = TRUE), 3),
    .groups = 'drop'
  ) %>%
...
write.csv(summary_stats,
          file      = "Fig_2b_Summary_C_peptide.csv",
          row.names = FALSE)
cat("✓ Summary statistics exported to Fig_2b_Summary_C_peptide.csv\n")

# ============================================================
# 8) Calculate C-peptide changes (Δ C-peptide)
# ============================================================
cat("\n=== CALCULATING C-PEPTIDE CHANGES ===\n")

cpeptide_changes_list <- list()
...
for (comp in my_comparison_weeks) {
  ...
  cpeptide_changes_temp <- dat %>%
    filter(Week_num %in% c(week1, week2)) %>%
    select(Replicate, Group, Week_num, C_peptide) %>%
    pivot_wider(names_from  = Week_num,
                values_from = C_peptide,
                names_prefix = "Week_") %>%
    mutate(
      !!comp_name := .data[[paste0("Week_", week2)]] -
        .data[[paste0("Week_", week1)]],
      !!pct_name  := round(((.data[[paste0("Week_", week2)]] -
                               .data[[paste0("Week_", week1)]]) /
                              .data[[paste0("Week_", week1)]]) * 100, 2)
    ) %>%
    select(Replicate, Group, all_of(c(comp_name, pct_name)))
  
  cpeptide_changes_list[[paste0(week1, "_", week2)]] <- cpeptide_changes_temp
}

cpeptide_changes <- cpeptide_changes_list[[1]]
for (i in 2:length(cpeptide_changes_list)) {
  cpeptide_changes <- cpeptide_changes %>%
    full_join(cpeptide_changes_list[[i]], by = c("Replicate", "Group"))
}

write.csv(cpeptide_changes,
          file      = "Fig_2b_C_peptide_Changes.csv",
          row.names = FALSE)
cat("✓ C-peptide changes exported to Fig_2b_C_peptide_Changes.csv\n")

# ============================================================
# 9) Summary of C-peptide changes by Group
# ============================================================
cat("\n=== SUMMARY OF C-PEPTIDE CHANGES BY GROUP ===\n")

cpeptide_change_summary <- cpeptide_changes %>%
  group_by(Group) %>%
  summarise(
    n               = n(),
    Mean_Delta_0_6  = round(mean(Delta_0_6,  na.rm = TRUE), 3),
    SEM_Delta_0_6   = round(sd(Delta_0_6,    na.rm = TRUE) / sqrt(n()), 3),
    Mean_Delta_6_12 = round(mean(Delta_6_12, na.rm = TRUE), 3),
    SEM_Delta_6_12  = round(sd(Delta_6_12,   na.rm = TRUE) / sqrt(n()), 3),
    Mean_Delta_0_12 = round(mean(Delta_0_12, na.rm = TRUE), 3),
    SEM_Delta_0_12  = round(sd(Delta_0_12,   na.rm = TRUE) / sqrt(n()), 3),
    .groups = 'drop'
  )

cat("\nSummary of C-peptide changes:\n")
print(cpeptide_change_summary)

write.csv(cpeptide_change_summary,
          file      = "Fig_2b_Summary_C_peptide_Changes.csv",
          row.names = FALSE)
cat("✓ C-peptide change summary exported to Fig_2b_Summary_C_peptide_Changes.csv\n")
...
cat(rep("=", 60), "\n", sep = "")
cat("\nGenerated files:\n")
cat(" 1. Fig_2b_Normality_Tests.csv         (Shapiro-Wilk — informational only)\n")
cat(" 2. Fig_2b_Statistical_Tests.csv       (paired t-tests, raw p-values)\n")
cat(" 3. Fig_2b_Boxplot_C_peptide.png       (600 dpi)\n")
cat(" 4. Fig_2b_Boxplot_C_peptide.svg       (vector format)\n")
cat(" 5. Fig_2b_Summary_C_peptide.csv       (descriptive statistics by week)\n")
cat(" 6. Fig_2b_C_peptide_Changes.csv       (individual-level \u0394 C-peptide)\n")
cat(" 7. Fig_2b_Summary_C_peptide_Changes.csv (group-level \u0394 C-peptide)\n")
cat("\nFigure dimensions: 8.5 cm (width) \u00d7 5.7 cm (height)\n")
cat("\nVISUAL APPEARANCE:\n")
cat(" - Matched to Fig 1b/2b for consistency\n")
...
cat(" - Replicate column used as individual animal ID for pairing\n")
...
cat(" C-peptide is a marker of insulin secretion\n")
cat(" Higher C-peptide indicates preserved beta-cell function\n")
cat(" Changes reflect pancreatic beta-cell response to DSS treatment\n")
cat(rep("=", 60), "\n", sep = "")
