# ============================================================
# Kidney Gene Expression Analysis - Fig_4e
...
# ✅ DATA QUALITY NOTES:
# - Tcf19 NOT AVAILABLE in kidney data (replaced with Insr)
# - Glut4 EXCLUDED due to extensive missing data (58% NA)
# - All 4 genes included have complete or near-complete data
...
# Filter for the 4 selected genes
dat_raw <- dat_raw %>%
  filter(Gene %in% genes_fig5b) %>%
...
dat_raw$Gene <- factor(dat_raw$Gene, levels = genes_fig5b)

cat("Total observations (raw, including NA):", nrow(dat_raw), "\n")
cat("Genes:", paste(genes_fig5b, collapse = ", "), "\n")

# Check for NA values by gene
cat("\nNA values per gene:\n")
na_summary <- dat_raw %>%
  group_by(Gene) %>%
  summarise(
...
# Calculate CTRL mean for each gene (excluding NA)
ctrl_means <- dat_raw %>%
  filter(Group == "CTRL") %>%
  group_by(Gene) %>%
  summarise(
...
if (nrow(dat_raw) > nrow(dat_fc)) {
  cat("Observations removed due to NA:", nrow(dat_raw) - nrow(dat_fc), "\n")
}

# Check sample sizes after removing NA
cat("\nSample sizes per gene per group (after removing NA):\n")
sample_sizes <- table(dat_fc$Gene, dat_fc$Group)
print(sample_sizes)
...
ggsave(
  filename = "Fig_4e_FoldChange_Kidney_GeneExpr.png",
  plot = plot_fig5b,
...
ggsave(
  filename = "Fig_4e_FoldChange_Kidney_GeneExpr.svg",
  plot = plot_fig5b,
  scale = 1, width = 8, height = 6,
...
write.csv(
  posthoc_result,
  file = "Fig_4e_Posthoc_FoldChange_BH_corrected.csv",
  row.names = FALSE
)
...
write.csv(
  summary_stats,
  file = "Fig_4e_Summary_FoldChange_Kidney_GeneExpr.csv",
  row.names = FALSE
)
...
write.csv(
  ...
  file = "Fig_4e_FoldChange_Data.csv",
  row.names = FALSE
)
...
# ============================================================
cat("\n", rep("=", 70), "\n", sep = "")
cat("ANALYSIS COMPLETE - KIDNEY GENES (FOLD CHANGE)\n")
cat(rep("=", 70), "\n", sep = "")

cat("\nGenerated files:\n")
cat("  Figures:\n")
cat("    - Fig_4e_FoldChange_Kidney_GeneExpr.png (600 dpi, 8×6 cm)\n")
cat("    - Fig_4e_FoldChange_Kidney_GeneExpr.svg (vector format)\n")
cat("  Data tables:\n")
cat("    - Fig_4e_Posthoc_FoldChange_BH_corrected.csv\n")
cat("    - Fig_4e_Summary_FoldChange_Kidney_GeneExpr.csv\n")
cat("    - Fig_4e_FoldChange_Data.csv\n")

cat("\nKey features:\n")
cat("  ✓ Log2(Fold change) relative to CTRL baseline\n")
cat("  ✓ facet_grid layout (4 panels, 1 row)\n")
...
cat("\nGenes analysed:\n")
cat("  ", paste(genes_fig5b, collapse = ", "), "\n")

cat("\nData quality:\n")
cat("  ✓ All 4 genes have complete or near-complete data\n")
cat("  ✓ Tcf19 replaced with Insr (Tcf19 not available in kidney)\n")
cat("  ✓ Glut4 excluded due to extensive missing data (58% NA)\n")
