# ============================================================
# Liver Gene Correlation Analysis - Fig_4c
# Pearson Correlation Matrix between Genes
# Diabetologia Journal Submission
# Five genes: Tbc1d4, Glut1, Zfand6, Tcf19, Ghrhr
...
genes_fig4c <- c("Tbc1d4", "Glut1", "Zfand6", "Tcf19", "Ghrhr")

# ============================================================
# 3) Import and prepare data
# ============================================================
cat("\n=== LOADING LIVER GENE EXPRESSION DATA ===\n")
dat_raw <- read.csv("Fig_4b_Data_Liver_GeneExpr.csv", stringsAsFactors = FALSE)

dat <- dat_raw %>%
  filter(Gene %in% genes_fig4c) %>%
...
cat("Genes:", paste(genes_fig4c, collapse = ", "), "\n")
...
plot_data$Gene1 <- factor(plot_data$Gene1, levels = genes_fig4c)
plot_data$Gene2 <- factor(plot_data$Gene2, levels = rev(genes_fig4c))
...
ggsave(
  filename = "Fig_4c_Correlation_Matrix_Liver.png",
  plot     = p_heatmap,
...
ggsave(
  filename = "Fig_4c_Correlation_Matrix_Liver.svg",
  plot     = p_heatmap,
  scale    = 1, width = 7.5, height = 6,
...
write.csv(cor_export, "Fig_4c_Correlation_Matrix_Liver.csv",    row.names = FALSE)
write.csv(p_export,   "Fig_4c_PValue_Matrix_Liver.csv",         row.names = FALSE)
write.csv(n_export,   "Fig_4c_N_Matrix_Liver.csv",              row.names = FALSE)
...
write.csv(pairwise_cor, "Fig_4c_Pairwise_Correlations_Liver.csv", row.names = FALSE)
...
# ============================================================
cat("\n", rep("=", 70), "\n", sep = "")
cat("CORRELATION ANALYSIS SUMMARY - LIVER GENES\n")
cat(rep("=", 70), "\n", sep = "")
...
cat(rep("=", 70), "\n", sep = "")
cat("Figures:\n")
cat("  - Fig_4c_Correlation_Matrix_Liver.png (600 dpi, 7.5×6 cm)\n")
cat("  - Fig_4c_Correlation_Matrix_Liver.svg (vector format)\n")
cat("\nData tables:\n")
cat("  - Fig_4c_Correlation_Matrix_Liver.csv    (correlation coefficients)\n")
cat("  - Fig_4c_PValue_Matrix_Liver.csv         (p-values)\n")
cat("  - Fig_4c_N_Matrix_Liver.csv              (n per pair)\n")
cat("  - Fig_4c_Pairwise_Correlations_Liver.csv (pairwise results with n)\n")
cat("\nInterpretation:\n")
