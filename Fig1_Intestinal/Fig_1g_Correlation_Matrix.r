# ============================================================
# Large Intestine Gene Correlation Analysis - Fig_1g
...
genes_fig1g <- c("Mafa", "Glp1r", "Insr", "Zfand6", "Tcf19")
...
cat("Genes:", paste(genes_fig1g, collapse = ", "), "\n")
...
plot_data$Gene1 <- factor(plot_data$Gene1, levels = genes_fig1g)
plot_data$Gene2 <- factor(plot_data$Gene2, levels = rev(genes_fig1g))
...
ggsave(
  filename = "Fig_1g_Correlation_Matrix.png",
  plot     = p_heatmap,
...
ggsave(
  filename = "Fig_1g_Correlation_Matrix.svg",
  plot     = p_heatmap,
  scale    = 1, width = 7.5, height = 6,
...
write.csv(cor_export, "Fig_1g_Correlation_Matrix.csv",    row.names = FALSE)
write.csv(p_export,   "Fig_1g_PValue_Matrix.csv",         row.names = FALSE)
write.csv(n_export,   "Fig_1g_N_Matrix.csv",              row.names = FALSE)
...
write.csv(pairwise_cor, "Fig_1g_Pairwise_Correlations.csv", row.names = FALSE)
...
cat(rep("=", 70), "\n", sep = "")
cat("Figures:\n")
cat("  - Fig_1g_Correlation_Matrix.png (600 dpi, 7.5×6 cm)\n")
cat("  - Fig_1g_Correlation_Matrix.svg (vector format)\n")
cat("\nData tables:\n")
cat("  - Fig_1g_Correlation_Matrix.csv    (correlation coefficients)\n")
cat("  - Fig_1g_PValue_Matrix.csv         (p-values)\n")
cat("  - Fig_1g_N_Matrix.csv              (n per pair)\n")
cat("  - Fig_1g_Pairwise_Correlations.csv (pairwise results with n)\n")
cat("\nInterpretation:\n")
