# ============================================================
# Spleen Gene Expression Analysis - Fig_3c
...
# 3) Define spleen genes for Fig_3c
genes_fig3c <- c("Mafa", "Insr", "Glut1", "Zfand6", "Tcf19")
...
dat_raw$Gene <- factor(dat_raw$Gene, levels = genes_fig3c)

cat("Total observations (raw):", nrow(dat_raw), "\n")
cat("Genes:", paste(genes_fig3c, collapse = ", "), "\n")
...
ggsave(
  filename = "Fig_3c_FoldChange_Spleen_GeneExpr.png",
  plot = plot_fig3c,
...
ggsave(
  filename = "Fig_3c_FoldChange_Spleen_GeneExpr.svg",
  plot = plot_fig3c,
  scale = 1, width = 9.5, height = 6,
...
write.csv(
  posthoc_result,
  file = "Fig_3c_Posthoc_FoldChange_BH_corrected.csv",
  row.names = FALSE
)
...
write.csv(
  summary_stats,
  file = "Fig_3c_Summary_FoldChange_Spleen_GeneExpr.csv",
  row.names = FALSE
)
...
write.csv(
  ...
  file = "Fig_3c_FoldChange_Data.csv",
  row.names = FALSE
)
...
# ============================================================
cat("\n", rep("=", 70), "\n", sep = "")
cat("ANALYSIS COMPLETE - SPLEEN GENES (FOLD CHANGE)\n")
cat(rep("=", 70), "\n", sep = "")
cat("\nGenerated files:\n")
cat("  Figures:\n")
cat("    - Fig_3c_FoldChange_Spleen_GeneExpr.png (600 dpi, 9.5×6 cm)\n")
cat("    - Fig_3c_FoldChange_Spleen_GeneExpr.svg (vector format)\n")
cat("  Data tables:\n")
cat("    - Fig_3c_Posthoc_FoldChange_BH_corrected.csv\n")
cat("    - Fig_3c_Summary_FoldChange_Spleen_GeneExpr.csv\n")
cat("    - Fig_3c_FoldChange_Data.csv\n")
cat("\nKey features:\n")
...
cat("\nGenes analysed:\n")
cat("  ", paste(genes_fig3c, collapse = ", "), "\n")
cat("\nSample size: n=4 per group per gene (complete data)\n")
cat(rep("=", 70), "\n", sep = "")
